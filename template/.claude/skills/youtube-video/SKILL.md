...
### 1. 選題
- **先判斷今天是不是「系列日」**：心跳訊息若出現「今天是系列日」，或工作區有 `SERIES.md` 且今天是它指定的出片日 → **今天走系列，不要用下面的一般選題流程、不要動 MEMORY 的待發題庫**。改成：讀完 `SERIES.md` → 依它的「選題順序」與「已講清單」挑今天的題 → 照它指定的腳本型態寫 → 標題與描述套它的 SEO 規則 → 發完把本集補進 SERIES.md 的已講清單。**做完這條就跳到第 2 步，以下一般選題不適用。**
- （以下為非系列日的一般選題）讀 `MEMORY.md` 的「已發布影片清單」，避免重複。
...
### 3.5 生插圖（每支片 4 張，Q 版矽膠風）
**先讀工作區的 `ILLUST.md`**，照它的風格模板與用途分類生圖，再接到第 4 步。
- 走 `draw-gpt`（ChatGPT 月費，不花 API 錢）：一張約 40~60 秒，4 張約 3~4 分鐘。
- **建議分配**：標題卡用吉祥物，中間 4 段各配一張插圖。
- 風格模板、呼叫指令、接進字卡的參數位置全都寫在 `ILLUST.md`，不要自己另編風格。
- ⚠️ **提示詞一律不要讓模型寫字**（中文必錯），所有文字交給 `make_slide` 疊。
- ⚠️ 生圖指令要**前景跑完**（同本檔開頭的鐵則）；某張 `FAIL` 就那段改用純文字字卡。
- 🛑 **連續 2 張 `FAIL` → 判定額度用光／服務不可用，本支片完全放棄插圖，整支回到原本的純文字字卡流程**，不要再試第 3 張、不要重試等待（每張要等 40~60 秒，硬試會拖到發片失敗）。放棄後在 MEMORY「下次改進方向」記一行。**發片永遠優先於插圖。**
...
- **要放插圖的字卡**：第 4 參數改傳 `"-"`（代表不要吉祥物），第 5 參數給插圖路徑 —— 版型會自動切成「標題 → 插圖 → 精簡內文」。有插圖時**內文要縮到 1~2 個短重點**，否則會被擠掉。詳見 `ILLUST.md`。
