...
## ⛔ 鐵則：產線指令一律前景跑完，做到發布才准結束
配音/合成/上傳這些指令要跑好幾分鐘，**絕對不要**把它們丟到背景執行後就說「等完成通知」結束回合——在心跳的 `claude -p` 模式下，回合一結束程序就退出，背景工作會被孤兒化、整條產線斷頭（曾因此整天斷更）。正確做法：每個指令**前景執行、設足夠的 timeout（narrate/make_video/upload 各給 10 分鐘）**，等它印出 OK 再走下一步；若工具自動把指令轉入背景，就留在回合內等它完成，**在 MEMORY.md 寫入今天的 `<!-- PUBLISHED:… -->` 標記之前，絕不結束回合**。
...
- **描述 SEO（前兩行最重要，搜尋/推薦只吃得到前兩行）**：
  ...
  - 中間：3~5 句補充內容、可放「上一集／系列」連結；**加一行互動問題**（例：「你還想看哪個 AI 名詞？留言告訴阿遠！」）催留言。
  - 結尾固定加 hashtag，**第一個務必是 `#Shorts`**（幫 YouTube 歸類為短影音），再接 `#AI #人工智慧 #科普 #<主題英文> #<主題中文>`（例 `#Shorts #AI #LLM #大型語言模型`）。
  - 標題結尾也可加 `#Shorts`（≤100 字元內）。
...
### 8.5 加入系列播放清單（選用，拉長追看）
```
python pipeline\add_to_playlist.py <videoId>
```
- 把今天的影片加進「AI 名詞白話系列」播放清單（找不到會自動建立），讓觀眾一支接一支看。
- **需先做過一次性 `youtube` 授權**（會產生 token_manage.json）；若該檔不存在就跳過這步、不要卡住流程。
